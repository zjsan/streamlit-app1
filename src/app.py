...
class App:
    def __init__(self):
        self.active_status = 0 #global variable to store the active status of the user 

    def set_active_status(self):
       self.active_status = 1
       return self.active_status
    
    def unset_active_status(self):
       self.active_status = 0
       return self.active_status

test = App()#creating instance of the class
...
def show_main_section():
     with main_section:
        ...
        driver = test.set_active_status()
       # st.write(f"Active Status value: {active_status}")#
        st.write(driver)
        st.write(f"Welcome, {st.session_state['email']}!")
        st.write(f"User session state value: {st.session_state['user']}")#for debugging
        st.write(f"Active Status: {driver}")#for debuggin
        #while True:#not executing?
        st.write(test.set_active_status())
        ...
        def main():
            ...
            with input_container:
                ...
                if  additional_prompts:
                    ...
                    if st.button("Submit") and user_message != None:
...
def showlogout_page():
   ...
   # initial_login_email =  st.session_state['email']#use for logout
    ...
    auth_section.empty()
    with logout_section:
        st.sidebar.button('Logout', key='logout', on_click=logout_clicked(initial_login_email))
